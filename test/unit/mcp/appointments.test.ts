...
//   - the window and the `providers` argument apply to portal items too
...
describe("upcoming portal visits", () => {
  it("returns a stored upcoming visit in the appointment shape, marked as the portal's", async () => {
    world.state.portalVisits.set(PROVIDER_A, [
...
describe("one visit, one item", () => {
  ...
  it("never merges across providers", async () => {
    ...
    expect(july.map((item) => item.provider)).toStrictEqual([NAME_A, NAME_B]);
  });
});
...
describe("arguments", () => {
  ...
  it("narrows portal visits to the providers asked for", async () => {
    const answer = await callTool(world.client, "get_appointments", { providers: [NAME_B] });
...
describe("the policy reaches portal items", () => {
  ...
  it("drops portal items with a resource rule on Encounter, and with their provider", async () => {
    ...
    world.state.rules = rules({ rule_type: "provider", target: PROVIDER_A });
