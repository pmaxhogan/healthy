...
import { clock, rawColumn, resetDb, seedHealthSystem, testRepos } from "./helpers.ts";
...
describe("oauth_states.put", () => {
  ...
  it("is rejected by the schema when kind and health system disagree", async () => {
    // A CHECK constraint, not a code path: an Epic authorization is always
    // against one health system and a Google one never is.
    const repos = testRepos();
    const healthSystemId = await seedHealthSystem(repos);
    ...
    await expect(
      repos.oauthStates.put({ kind: "google", healthSystemId, codeVerifier: "v", ttlMs: 600_000 }),
    ).rejects.toThrow();
  });
});
...
describe("oauth_states.purgeExpired", () => {
  ...
  it("cascades when its health system goes away", async () => {
    // ON DELETE CASCADE on health_system_id: a hard-deleted health system cannot leave an
    // in-flight authorization behind that would complete against nothing.
    const repos = testRepos();
    const healthSystemId = await seedHealthSystem(repos);
    await repos.oauthStates.put({
      kind: "epic",
      healthSystemId,
      codeVerifier: "v",
      ttlMs: 600_000,
    ...
    await repos.ctx.db
      .prepare("DELETE FROM health_systems WHERE id = ?")
      .bind(healthSystemId)
      .run();
